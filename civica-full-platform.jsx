...
function AgentTerminal({ logs }) {
  ...
  return (
    <div style={{ background: "#000", border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, height: 200, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ fontSize: 10, color: C.green, letterSpacing: 2, marginBottom: 8, display: "flex", justifyContent: "space-between" }}>
        <span>>_ AGENT_SYSTEM_LOGS</span>
        <span className="blink">● LIVE</span>
      </div>
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", fontFamily: "monospace", fontSize: 11, display: "flex", flexDirection: "column", gap: 6 }}>
        {logs.map(l => (
          <div key={l.id} style={{ display: "grid", gridTemplateColumns: "60px 90px 1fr", gap: 8, color: C.textSub }}>
            <span style={{ color: C.textDim }}>{l.time}</span>
            <span style={{ color: l.agent === "TRIAGE_AGENT" ? C.amber : l.agent === "GOV_AGENT" ? C.blue : C.green }}>{l.agent}</span>
            <span style={{ color: C.text }}>{l.action}: {l.details}</span>
          </div>
        ))}
      </div>
    </div>
...
/* ═══════════════════════════════════════════════════════════
   CITIZEN PORTAL
═══════════════════════════════════════════════════════════ */
function CitizenPortal({ issues, onReport, onVote, socialFeed, onAnalyze, analysisResult, onAskInsight, insightResult }) {
  ...
  return (
    <div style={{ padding: "0 0 32px" }}>
      ...
      {/* Live Social Ticker */}
      <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 12, padding: "10px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12, overflow: "hidden" }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: C.blue, whiteSpace: "nowrap" }}>LIVE SOCIAL FEED</div>
        <div style={{ width: 1, height: 16, background: C.border }} />
        <div style={{ display: "flex", flexDirection: "column", gap: 2, flex: 1 }}>
          {socialFeed.slice(0, 1).map(t => (
            <div key={t.id} className="fadeUp" style={{ fontSize: 12, color: C.textDim }}>
              <span style={{ color: C.text, fontWeight: 700 }}>{t.user}</span>: {t.text} <span style={{ fontSize: 10, color: t.sentiment === 'neg' ? C.red : C.green }}>({t.sentiment})</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {filtered.map((issue, idx) => {
          ...
          const votes = issue.votes; // Server authority
      ...
      {showReport && <ReportModal issues={issues} onClose={() => setShowReport(false)} onSubmit={onReport} onAnalyze={onAnalyze} analysisResult={analysisResult} />}
      {selected && <IssueDetailModal issue={selected} onClose={() => setSelected(null)} onAsk={onAskInsight} insightResult={insightResult} />}
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════
   AUTHORITY DASHBOARD
═══════════════════════════════════════════════════════════ */
function AuthorityDashboard({ issues, agentLogs = [], onAskAi, aiPlan }) {
  ...
  useEffect(() => {
    if (aiPlan && selectedIssue && aiPlan.issueId === selectedIssue.id) {
      setAiAction(aiPlan.text);
      setAiLoading(false);
    }
  }, [aiPlan, selectedIssue]);
      ...
      <div style={{ display: "grid", gridTemplateColumns: selectedIssue ? "1fr 380px" : "1fr", gap: 16 }}>
            ...
            return (
              ...
              >
                ...
                <div style={{ padding: "0 8px", display: "flex", gap: 6, alignItems: "center" }}>
                  <button onClick={e => { e.stopPropagation(); setSelectedIssue(issue); handleAskAi(issue); }} style={{
        ...
        {selectedIssue && (
          <div style={{ background: C.card, border: `1px solid ${C.teal}44`, borderRadius: 14, padding: 18, alignSelf: "start", position: "sticky", top: 80 }}>
            ...
            {!aiAction && !aiLoading && (
              <button onClick={() => handleAskAi(selectedIssue)} style={{
...
/* ═══════════════════════════════════════════════════════════
   ANALYTICS CENTER
═══════════════════════════════════════════════════════════ */
function AnalyticsCenter({ issues, liveStats = [] }) {
  // Chart data
  ...
  // Use liveStats for the trend chart
  const trendData = liveStats.length > 0 ? liveStats : [{ time: "Now", risk: 50, active: 0, resolved: 0 }];
  ...
  const cityHealth = issues.length > 0 ? 100 - Math.round(issues.reduce((s, i) => s + i.civicScore, 0) / issues.length) : 100;
  const avgCivicScore = issues.length > 0 ? Math.round(issues.reduce((s, i) => s + i.civicScore, 0) / issues.length) : 0;
  ...
  return (
    <div style={{ padding: "0 0 32px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "16px 18px" }}>
          <div style={{ fontSize: 10, color: C.blue, letterSpacing: 2, fontWeight: 700, marginBottom: 14 }}>📈 LIVE RISK MONITOR (REAL-TIME)</div>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={trendData} margin={{ left: -20 }}>
              ...
              <XAxis dataKey="time" tick={{ fill: C.textDim, fontSize: 10, fontFamily: FONT }} interval={4} />
              <YAxis domain={[0, 100]} tick={{ fill: C.textDim, fontSize: 10, fontFamily: FONT }} />
              <Tooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={{ fontSize: 10, fontFamily: FONT, color: C.textDim }} />
              <Area type="monotone" dataKey="risk" stroke={C.red} fill="url(#riskGrad)" name="Risk Index" isAnimationActive={false} />
              <Area type="monotone" dataKey="active" stroke={C.blue} fill="transparent" name="Active Issues" isAnimationActive={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
...
═══════════════════════════════════════════════════════════ */
function ReportModal({ issues, onClose, onSubmit, onAnalyze, analysisResult }) {
...
═══════════════════════════════════════════════════════════ */
function IssueDetailModal({ issue, onClose, onAsk, insightResult }) {
...
export default function App() {
  ...
  const [simEnabled, setSimEnabled] = useState(true); // Controls UI only now
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => addToast("Connected", "Live stream active", "info"));
    
    socket.on("update-issues", (data) => {
      setIssues(data.sort((a, b) => b.civicScore - a.civicScore));
    });

    socket.on("new-alert", (alert) => {
      addToast(alert.title, alert.msg, "critical");
    });
    ...
    socket.on("social-stream", (tweet) => {
      setSocialFeed(prev => [tweet, ...prev].slice(0, 10));
    });

    socket.on("stats-update", (stat) => {
      setLiveStats(prev => [...prev, stat].slice(-20)); // Keep last 20 points for graph
    });

    socket.on("ai-plan-response", (plan) => {
      setAiPlan(plan);
    });

    socket.on("report-analysis-result", (res) => {
      setAnalysisResult(res);
    });

    socket.on("issue-insight-response", (res) => {
  ...
  const handleReport = (form, aiScore) => {
    ...
    socketRef.current.emit("report-issue", newIssue);
  };
  ...
  const handleAskAi = (issue) => {
    socketRef.current.emit("ask-ai-plan", issue);
  };

  const handleAnalyzeReport = (form) => {
    socketRef.current.emit("analyze-report", form);
  };

  const handleAskInsight = (data) => {
    socketRef.current.emit("ask-issue-insight", data);
  };
  ...
  return (
    <div style={{ background: C.bg0, minHeight: "100vh", fontFamily: FONT, color: C.text }}>
      ...
      <header style={{
        ...
        <div style={{ width: "100%", display: "flex", alignItems: "center", gap: 0, height: 56 }}>
          {/* Logo */}
          ...
          <button style={{
            ...
            borderRadius: 20, padding: "6px 12px", fontSize: 10, fontWeight: 700, color: C.green,
            cursor: "default", marginRight: 16, display: "flex", alignItems: "center", gap: 6
          }}>
            <span style={{ display: "block", width: 6, height: 6, borderRadius: "50%", background: C.green, boxShadow: `0 0 8px ${C.green}` }} />
            SERVER ONLINE
          </button>
      ...
      {/* Page content */}
      <main style={{ width: "100%", padding: "24px 32px" }} className="fadeUp">
        {section === "citizen" && <CitizenPortal issues={issues} onReport={handleReport} onVote={handleVote} socialFeed={socialFeed} onAnalyze={handleAnalyzeReport} analysisResult={analysisResult} onAskInsight={handleAskInsight} insightResult={insightResult} />}
        {section === "authority" && <AuthorityDashboard issues={issues} agentLogs={agentLogs} onAskAi={handleAskAi} aiPlan={aiPlan} />}
        {section === "analytics" && <AnalyticsCenter issues={issues} liveStats={liveStats} />}
